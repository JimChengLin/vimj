...
'use strict';
// Hook
HTMLElement.prototype._1995_addEventListener = HTMLElement.prototype.addEventListener;
HTMLElement.prototype.addEventListener = function (type, listener, userCapture) {
    if (this.tagName.match(/^(DIV|LI|I|LABEL|SPAN)$/) && type.match(/mouse|click/)) {
        Page.clickables.add(this);
    }
    ...
    addEventListener('keydown', (event) => {
        ...
        function shouldTab() {
            if (activeElement && activeElement.tagName === 'INPUT') {
                var last = $(activeElement).closest('form').find('input[type="text"], input[type="password"]').last();
                return last.length && !last.is(activeElement);
            }
            return false;
        }
    }, true);
    ...
    if (location.href.includes('youtube.com')) {
        setInterval(() => {
            if (location.href === 'https://www.youtube.com/') {
                $('ytd-item-section-renderer:has(.ytd-subscribe-button-renderer)').remove();
                $('ytd-item-section-renderer:has(span:contains(Live Gaming for You))').remove();
                $('ytd-item-section-renderer:has(span:contains(Live recommendations))').remove();
                $('ytd-item-section-renderer:has(yt-formatted-string:contains(Recommended channel for you))').remove();
            }
        }, 100);
    } else if (location.href.includes('zhihu.com')) {
        setInterval(() => {
            $('div.Card.Banner, div.TopstoryItem--advertCard, div.AnalyticsCountDelta, .Pc-word, .Pc-card, div.css-u2c28s').hide();
        }, 100);
    } else if (location.href.includes('36kr.com')) {
        setInterval(() => {
            $('.information-flow-item:has(.home-flow-article-logo)').hide();
        }, 100);
    }
}
...
var Page = {
    ...
    linkHint: () => {
        ...
        function getElements() {
            ...
            var clickables = $(Array.from(Page.clickables));
            return purify(elements, clickables.add(clickables.find('div, li, i, label, span')).addClass('_strict'));

            function purify(elements, clickables) {
                var length = 16;
                var minTop = -8;
                ...
                function isDisplayed(element) {
                    ...
                    if (style.opacity === '0' || (element.classList.contains('_strict') &&
                        style.cursor.search(/pointer|text|grab/) === -1)) {
                        return false;
                    }

                    var rect = element.getClientRects()[0];
                    if (rect && rect.left >= 0 && rect.top >= minTop &&
                        rect.right <= innerWidth && rect.bottom <= innerHeight) {
                        element._left = rect.left;
                        element._top = Math.max(0, rect.top);
                        ...
                        for (var i = 0; i < positions.length; i++) {
                            targetElement = document.elementFromPoint(positions[i][0], positions[i][1]);
                            if (targetElement === element || element.contains(targetElement)) {
                                return true;
                            }
                        }
                        if (element.tagName === 'INPUT' && targetElement && targetElement.tagName !== 'INPUT') {
                            var a = xPath(element);
                ...
                var r = $(elements).add(clickables);
                var dups = r.filter('a')
                            .find('._strict');
                var substitutes = $('a').not(r)
                                        .find('> *')
                                        .filter((i, elem) => isDisplayed(elem)).get().reverse().filter(isExclusive);
                return r.not(dups).add(substitutes);
            }
        }
        ...
        function getHints(elements) {
            ...
            for (i = 0; i < elements.length && availableChar === 'F'; i++) {
                ...
                if ((element.tagName === 'INPUT' &&
                    element.type.search(/^(button|checkbox|file|hidden|image|radio|reset|submit)$/) === -1)
    ...
    scrollTop: (offset) => {
        var target = Scroller.findScrollTarget();
    ...
    click: (element) => {
        if ((element.tagName === 'INPUT' && element.type.search(/^(button|checkbox|file|hidden|image|radio|reset|submit)$/) === -1)
            || element.hasAttribute('contenteditable') || element.tagName === 'TEXTAREA') {
            element.focus();
            ...
            for (var i = 0; i < names.length; i++) {
                element.dispatchEvent(new MouseEvent(names[i], { bubbles: true, clientX: element._left, clientY: element._top }));
            }
        }
    },
    ...
    isCommand: (event) => {
        ...
        return !event.ctrlKey && !event.altKey && !isInput && isUseful;
    }
};
...
var Tree = {
    ...
    getLeft: (node) => {
        if (node.left) {
            ...
            return node.left = Tree.create(node.from, (node.from + node.to) / 2);
        }
    },
    ...
    getRight: (node) => {
        if (node.right) {
            ...
            return node.right = Tree.create((node.from + node.to) / 2 + 1, node.to);
        }
    },
...
var Scroller = {
    findScrollTarget: () => {
        var target = Scroller.findScrollableUntilHTML(document.activeElement)
            || Scroller.findScrollableChild(document.activeElement)
            || Scroller.findScrollableChild(document.body);
        return target;
    },

    findScrollableUntilHTML: (element) => {
        while (element && !Scroller.isScrollable(element)) {
    ...
    findScrollableChild: (element) => {
        ...
        for (var child of element.querySelectorAll('*')) {
            if (Scroller.isScrollable(child)
                && (!scrollableChild || child.clientHeight > scrollableChild.clientHeight)) {
    ...
    isScrollable: (element) => {
        return element.scrollHeight > element.clientHeight
            ...
            && ((element.scrollTop += 1) !== 1 || (element.scrollTop += -1) !== -1);
    }
}
